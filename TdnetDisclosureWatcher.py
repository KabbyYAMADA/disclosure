...
#
# Des Securities #general
#slack = slackweb.Slack(url = "https://hooks.slack.com/services/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")

# Des Securities #disclosure 
slack = slackweb.Slack(url = "https://hooks.slack.com/services/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")
...
def IsMarketOpened():
    ...
    #
    dt_now = datetime.now()
    dt_market_open = datetime(dt_now.year, dt_now.month, dt_now.day, 8, 0, 0)
    dt_market_close = datetime(dt_now.year, dt_now.month, dt_now.day, 20, 0, 0)
...
while(True):
    ...
    if datanum > 0:
        ...
        for index, kaiji in df_t.iterrows():            
            ...
            #
            excludedic = ['定款', 'ガバナンス', '支配株主', '投資単位', 'ETF',
                        'ETN', '法定', '投信', '先物', '約款',
                        '信託', '異動', '自己株式', '公益財団', '届出書',
